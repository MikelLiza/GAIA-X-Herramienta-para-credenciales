...
async function actionVP() {
  ...
  /**
   * 1. VP ORIGINAL (sin compliance)
   */
  const vpOriginal = buildVerifiablePresentation({
    verifiableCredentials,
  });
  
  const vpOriginalPath = path.join(
    LOCAL_OUTPUT_DIR,
    `vp-${config.shortName}.json`
  );

  logger.info(`Writing VP (no compliance) -> ${vpOriginalPath}`);
  await writeFile(vpOriginalPath, vpOriginal);
